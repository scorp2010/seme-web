 ...
 */
package com.sem.zkclient.connection;
...
import com.sem.zkclient.exception.ZKTimeoutException;
import com.sem.zkclient.util.ExceptionUtil;
import java.io.File;
...
public class ZKConnectionImpl implements ZKConnection {
	 ...
	 * 
	 * @param servers
	 *            zookeeper服务地址， 格式采用，逗号分隔主机:端口号。 例如
	 *            "127.0.0.1:3000,127.0.0.1:3001,127.0.0.1:3002" 如果改变了根目录例如：
	...
	@Override
	public <T> T retryUntilConnected(Callable<T> callable) throws ZKInterruptedException, ZKTimeoutException,
	...
	/**
	 * 等待直到连接处于某个状态才停止，如果超时返回false，当正确处于某个状态返回true 这里使用到了EventLock的
	 * stateChangedCondition 条件， 如果当前状态不是期待的状态，则此时线程处于等待状态。
	 ...
	 * @return boolean
	 */
	private boolean waitForKeeperState(KeeperState keeperState, long timeout, TimeUnit timeUnit)
			throws ZKInterruptedException {
