...
public interface ZKConnection {
	...
	/**
	 * 尝试连接， 直到连接成功为止 对于连接失败，或者会话超时，会发起重新连接的请求，只到连接成功。
	 * 
	 * @param callable
	 * @return
	 ...
	 * @throws ZKException
	 *             所有的ZooKeeper异常发生会抛出此异常
	 * @throws RuntimeException
	 *             执行时异常
	 * @throws ZKTimeoutException
	 *             如果设置了超时时间operationRetryTimeoutInMillis，
	 *             则会在尝试时间超过operationRetryTimeoutInMillis时 抛出异常
	 * @return T
	 */
	<T> T retryUntilConnected(Callable<T> callable) throws ZKInterruptedException, ZKTimeoutException, ZKException,
			RuntimeException;
