 ...
 */
package com.sem.zkclient.watcher;

import com.sem.zkclient.ZKClient;
import com.sem.zkclient.event.ZKEvent;
import com.sem.zkclient.event.ZKEventThreadPool;
...
public class ZKWatcherProcess {
	...
	public void processStateChanged(WatchedEvent event) {
		...
		if (event.getState() == KeeperState.Expired) {
			try {
				...
				// 会话过期后重连出错，事件处理
				for (final ZKStateListener stateListener : listeners) {
					eventThreadPool.submit(new ZKEvent("Session establishment error[" + e + "] sent to "
							+ stateListener) {
						@Override
						public void run() throws Exception {
	...
	/**
	 * 触发所有的监听器，用于在会话失效后调用。 会话失效后，服务端会取消watch，
	...
	/**
	 * 对listeners进行分类整理，把{@link ZKNodeListener}类型的放入nodeListeners，把
	 * {@link ZKChildDataListener} 类型的放入childDataChangeListeners
	 * 
	 * @param listeners
	 * @param nodeListeners
