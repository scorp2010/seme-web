 ...
 */
package com.sem.zkclient.leader;
...
import com.sem.zkclient.lock.ZKDistributedDelayLock;
...
public class ZKLeaderDelySelector implements LeaderSelector {
	...
	/**
	 * 创建Leader选举对象 ZKLeaderSelector.
	 * 
	 * @param id
	 *            每个Leader选举的参与者都有一个ID标识，用于区分各个参与者。
	 * @param autoRequue
	 *            是否在由于网络问题造成与服务器断开连接后，自动参与到选举队列中。
	 * @param delayTimeMillis
	 *            延迟选举的时间，主要是针对网络闪断的情况，给Leader以重连并继续成为Leader的机会，一般5秒合适。
	 * @param client
	 *            ZKClient
	 * @param leaderPath
	 *            选举的路径
	 ...
	 *            成为Leader后执行的的监听器
	 */
	public ZKLeaderDelySelector(String id, Boolean autoRequue, Integer delayTimeMillis, ZKClient client,
			String leaderPath, ZKLeaderSelectorListener listener) {
