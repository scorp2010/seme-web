...
import com.sem.zkclient.listener.ZKNodeListener;
...
/**
 * 带延迟获取的分布式锁 此分布式锁主要针对网络闪断的情况。
 * 不带延迟功能的分布式锁：某个线程获取了分布式锁,在网络发生闪断，ZooKeeper删除了临时节点，那么就会释放锁。
 * 带延迟功能的分布式锁：例如设置了delayTimeMillis的值为5000
...
public class ZKDistributedDelayLock implements ZKLock {
	 ...
	 * 
	 * @param timeout
	 *            超时时间 如果超时间大于0，则会在超时后直接返回false。 如果超时时间小于等于0，则会等待直到获取锁为止。
	 * @return
	...
	public boolean lock(int timeout, int delayTimeMillis) {
		...
		try {
			...
			try {
				if (timeout > 0) {
					getLock = semaphore.tryAcquire(timeout, TimeUnit.MILLISECONDS);
				} else {
					semaphore.acquire();
					getLock = true;
				}
	...
	/**
	 * 设置锁存储的值，一定要唯一，且不允许为null 默认使用UUID动态生成
	 * 
	 * @param lockNodeData
	 * @return void
	 */
