...
public class ZKClientBuilder {
	...
	public ZKClient build() {
		...
		ZKClient zkClient = new ZKClient(servers, sessionTimeout, retryTimeout, zkSerializer, connectionTimeout,
				eventThreadPoolSize);
		return zkClient;
	}
	...
	/**
	 * 设置序列化类，可选. （默认实现:{@link SerializableSerializer}）
	 * 
	 * @param zkSerializer
	 * @return
	 * @return ZKClientBuilder
	...
	/**
	 * 设置会话失效时间，可选 （默认值：30000，实际大小ZooKeeper会重新计算大概在2 * tickTime ~ 20 * tickTime）
	 * 
	...
	/**
	 * 连接超时时间，可选。 默认值Integer.MAX_VALUE
	 * 
	...
	/**
	 * 重试超时时间，可选，主要用于ZooKeeper与服务器断开后的重连。 默认值-1，也就是没有超时限制
	 * 
	 * @param retryTimeout
	 * @return
	 * @return ZKClientBuilder
